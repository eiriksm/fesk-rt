/**
 * Core FESK (Frequency Shift Keying) processor
 * This module contains the DSP algorithm for detecting and decoding
 * frequency-encoded audio signals. It can be used in both real-time
 * (AudioWorklet) and batch/offline processing contexts.
 */

export class FeskCore {
  constructor(sampleRate) {
    ...
    // Callback for results - will be set by wrapper
  ...
  configure(config) {
    ...
    this.minToneSamples = Math.max(
      1,
      Math.round(
        ((Number.isFinite(minToneMs) ? minToneMs : 0) * this.sampleRate) / 1000,
      ),
    );
    this.minGapSamples = Math.max(
      1,
      Math.round(
        ((Number.isFinite(minGapMs) ? minGapMs : 0) * this.sampleRate) / 1000,
      ),
    );
    this.ignoreHeadSamples = Math.max(
      0,
      Math.round(
        ((Number.isFinite(ignoreHeadMs) ? ignoreHeadMs : 0) * this.sampleRate) /
          1000,
      ),
    );
    ...
    const envSamples = Math.max(1, (envMs * this.sampleRate) / 1000);
    this.energyDecay = Math.exp(-1 / envSamples);
    this.energyRise = 1 - this.energyDecay;
    this.energyEnv = 0;

    const hpHz = Number.isFinite(hpCutoffHz) ? hpCutoffHz : 600;
    this.hpAlpha = Math.exp((-2 * Math.PI * hpHz) / this.sampleRate);
    this.hpLastX = 0;
    this.hpLastY = 0;
  ...
  finalizeTone() {
    ...
    if (results.length && this.onCandidates) {
      this.onCandidates({
        pipeline: this.pipelineKey,
        results,
      });
   ...
   * @param {number} sample - Audio sample value
   */
   ...
   * @param {Float32Array|Array} samples - Array of audio samples
   */
  ...
  /**
   * Force finalization of any pending tone (useful for end-of-stream)
   */
  flush() {
    if (this.toneActive && this.toneSamples > 0) {
      this.finalizeTone();
    }
  }
